...
async function importTweets() {
  ...
  const displayName = tweetsData.account[0].account.accountDisplayName || username;
  ...
  for (let i = 0; i < tweetsData.tweets.length; i += CHUNK_SIZE) {
    ...
    // Process this chunk
    const processedTweets = processTweets(chunk, username, accountId, displayName);
    const threads = buildThreads(processedTweets);
    
    if (threads.length === 0) continue;

    // Generate embeddings
    process.stdout.write(`Generating embeddings for ${threads.length} threads... `);
    ...
    const workerPromises = Array.from({ length: NUM_WORKERS }, (_, workerIndex) => {
      const workerThreads = threads.filter((_, i) => i % NUM_WORKERS === workerIndex);
      const workerEmbeddings = embeddings.filter((_, i) => i % NUM_WORKERS === workerIndex);
      ...
      return new Promise<void>((resolve, reject) => {
        ...
        worker.onmessage = (event) => {
          ...
          if (response.success) {
            ...
            console.log(`Worker ${workerIndex + 1} completed ${response.count} threads`);
          } else {
            errorCount++;
            console.error(`Worker ${workerIndex + 1} failed:`, response.error);
          }
          resolve();
        };
...
function processTweets(
  ...
  console.log(`Filtered down to ${validTweets.length} tweets from ${rawTweets.length}`);
  return validTweets;
}
...
function buildThreads(tweets: Tweet[]) {
  ...
  console.log(`Built ${threads.length} threads. Total word count: ${wordCount}`);
